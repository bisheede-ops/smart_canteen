...
export default function PickupOrders() {
  ...
  const pickupListData = [
    ...
    ...(pickupOrders.length
      ? pickupOrders.map(o => ({ _type: "pickup",   _key: `pu-p-${o.id}`, done: false, order: o }))
    ...
    ...(pickedUpOrders.length
      ? pickedUpOrders.map(o => ({ _type: "pickup",   _key: `pu-d-${o.id}`, done: true,  order: o }))
  ...
  const deliveryListData = [
    ...
    ...(deliveryOrders.length
      ? deliveryOrders.map(o => ({ _type: "delivery", _key: `dl-p-${o.id}`, done: false, order: o }))
    ...
    ...(deliveredOrders.length
      ? deliveredOrders.map(o => ({ _type: "delivery", _key: `dl-d-${o.id}`, done: true,  order: o }))
  ...
  const renderItem = ({ item }) => {
    ...
    if (item._type === "pickup")  return <PickupCard   item={item.order} onMarkPickedUp={confirmMarkPickedUp} done={item.done} />;
    return                               <DeliveryCard item={item.order} onMarkReady={confirmMarkReady}       done={item.done} />;
  };
